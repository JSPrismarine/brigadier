...
describe('CommandDispatcherTest', () => {
    ...
    it('testCreateAndMergeCommands', () => {
        subject.register(
            literal('base').then(literal('foo').executes(command))
        );
        subject.register(
            literal('base').then(literal('bar').executes(command))
        );
            ...
            literal('test')
                ...
                .then(
                    argument('right', integer()).then(
                        argument('sub', integer()).executes(subCommand as any)
                    ) as any // FIXME
                )
        );
    ...
    it('testExecuteRedirectedMultipleTimes', () => {
        const concreteNode = subject.register(
            literal('actual').executes(command)
        );
        ...
        expect(parse.getContext().getNodes()[0].getRange()).to.deep.equal(
            parse.getContext().getRange()
        );
        ...
        expect(child1.getNodes()[0].getRange()).to.deep.equal(
            child1.getRange()
        );
        ...
        expect(child2.getNodes()[0].getRange()).to.deep.equal(
            child2.getRange()
        );
    ...
    it('testExecuteRedirected', () => {
        ...
        const redirectNode = subject.register(
            literal('redirected').fork(subject.getRoot(), mockedModifier)
        );
        ...
        assert.equal(parse.getContext().getRootNode(), subject.getRoot());
        expect(parent.getNodes()[0].getRange()).to.deep.equal(
            parent.getRange()
        );
