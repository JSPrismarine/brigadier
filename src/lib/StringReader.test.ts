...
describe('StringReader Test', () => {
    ...
    it('readQuotedString_noOpen', () => {
        try {
            ...
            assert.equal(
                ex.getType(),
                CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerExpectedStartOfQuote()
            );
    ...
    it('readQuotedString_noClose', () => {
        try {
            ...
            assert.equal(
                ex.getType(),
                CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerExpectedEndOfQuote()
            );
    ...
    it('readQuotedString_invalidEscape', () => {
        try {
            ...
            assert.equal(
                ex.getType(),
                CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerInvalidEscape()
            );
    ...
    it('expect_incorrect', () => {
        ...
        try {
            ...
            assert.equal(
                ex.getType(),
                CommandSyntaxException.BUILT_IN_EXCEPTIONS.readerExpectedSymbol()
            );
            assert.equal(ex.getCursor(), 0);
        }
    });
